...
async function run() {
  ...
  // test should match the DB scene before the test.
  //
  // liveHadData depends on whether the journal entry is linked
  // to the existing drawing record in this session's workspace
  // state — a test-fixture concern, not a fix concern.
  // The PASS gate only checks DB integrity.
  const dbUnchanged =
    post.elementCount === pre.elementCount;

  // Check the probe logs for evidence the guards actually fired:
  const echoSuppressedCount = msgs.filter((m) =>
    /echoSuppressed=true/.test(m.text)
  ).length;
  const mountEchoCount = msgs.filter((m) =>
    /mount-echo suppressed/.test(m.text)
  ).length;
  const emptyOverPopCount = msgs.filter((m) =>
    /empty-over-populated save REFUSED/.test(m.text)
  ).length;
  const guardsFired = echoSuppressedCount > 0 || mountEchoCount > 0 || emptyOverPopCount > 0;
  ...
  writeReport('T10-mount-echo', {
    ...
    diags: {
      ...
      echoSuppressed: echoSuppressedCount,
      mountEchoSuppressed: mountEchoCount,
      emptyOverPopulatedWarned: emptyOverPopCount,
      savingOrDirtyLines: savingLines.length,
      guardsFired,
      firstOnChangeLines: onChangeLines.slice(0, 6).map((m) => m.text),
      lastOnChangeLines: onChangeLines.slice(-4).map((m) => m.text),
    },
